...
function sendFile(fileName, range, response) {
    if (!checkIfValidFile(fileName))
        ...
        sendCompleteFile(fileName, response);
    else
        sendPartialFile(fileName, range, response);
}
...
exports.sendCompleteFile = sendCompleteFile;
function sendPartialFile(fileName, range, response) {
    ...
    if (isNaN(end))
        end = (start + chunkSize - 1) > (totalLength - 1) ? (totalLength - 1) : (start + chunkSize - 1);
    if (isNaN(start)) {
...
exports.sendPartialFile = sendPartialFile;
...
function getMimeFromFileName(fileName) {
    ...
    if (mimeNames[ext.toLowerCase()] == undefined)
        return 'application/octet-stream';
...
exports.checkIfValidFile = checkIfValidFile;
